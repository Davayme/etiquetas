...
def create_model(input_shape):
    model = tf.keras.Sequential([
        ...
        tf.keras.layers.Dense(96, activation='relu', input_shape=(input_shape,),
                             kernel_regularizer=tf.keras.regularizers.l2(0.002)),  # Aumentado L2
        tf.keras.layers.BatchNormalization(),
        tf.keras.layers.Dropout(0.3),  # Aumentado dropout
    ...
    model.compile(
        optimizer=tf.keras.optimizers.Adam(
            ...
            beta_2=0.999,
            epsilon=1e-07,
            amsgrad=True  # Activado para mayor estabilidad
        ),
